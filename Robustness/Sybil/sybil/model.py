...
class Sybil:
    ...
    def _predict(
        ...
        return_features : bool 
            If True, returns features for each serie. 
        Returns
        -------
        Prediction
        ...
        features: List[List, np.ndarray] = [] if return_features else None 
    ...
    def evaluate(
        ...
        print(f"scores has length {len(scores)}")
        print(len(scores[0]))
        labels = [serie.get_label(self._max_followup) for serie in series]
        print(f"censor status and time have lengths {len([label.censor_time for label in labels])}")
