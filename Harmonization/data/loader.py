...
class Loader(data.Dataset):
    def __init__(self, opt):
        ...
        self.scale = opt.get('scale', 1.0) # Scale by slice thickness
        self.ToTensor = ImgToTensor()


    def __getitem__(self, index):
        uid = self.uids[index]
        affine_in, header_in, in_data = read_data(uid, self.data_type)
        if header_in:
            metadata_out = os.path.join(self.out_dir, '--'.join(uid.lstrip('/').split('/')))
        ...
        out_dict = {'uid':'--'.join(uid.lstrip('/').split('/')), 'affine_info': torch.from_numpy(affine_in), 'dataroot_LR':in_data}
        if self.tar_folder:
